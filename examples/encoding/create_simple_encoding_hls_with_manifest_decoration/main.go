...
func main() {
	...
	keyframe := &models.Keyframe{
		Time:       floatToPtr(17.2),
		SegmentCut: boolToPtr(true),
	}

	keyframeResp, err := encodingS.AddKeyframe(*encodingResp.Data.Result.ID, keyframe)
	errorHandler(keyframeResp.Status, err)
	...
	customTag := &models.CustomTag{
		KeyframeID:   keyframeResp.Data.Result.ID,
		PositionMode: bitmovintypes.Keyframe,
		Data:         stringToPtr("#EXT-YOUR-TAG-HERE"),
	}

	customTagAudioResponse, err := hlsService.AddCustomTagToMediaInfo(*hlsManifestResp.Data.Result.ID, *audioMediaInfoResp.Data.Result.ID, customTag)
	errorHandler(customTagAudioResponse.Status, err)
	customTag1080pResponse, err := hlsService.AddCustomTagToStreamInfo(*hlsManifestResp.Data.Result.ID, *video1080pStreamInfoResponse.Data.Result.ID, customTag)
	errorHandler(customTag1080pResponse.Status, err)
	customTag720pResponse, err := hlsService.AddCustomTagToStreamInfo(*hlsManifestResp.Data.Result.ID, *video720pStreamInfoResponse.Data.Result.ID, customTag)
	errorHandler(customTag720pResponse.Status, err)
